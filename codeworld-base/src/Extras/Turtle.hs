...
module Extras.Turtle(
    Track, tracks, fd, bk, rt, lt, seth, setxy, home, pu, pd, overxy, sethome
...
import Extras.Op
import Extras.Util(run,repeat,foreach,forloop)
...
-- | Convert a sequence of Turtle commands into a list of Tracks
tracks :: [Turtle -> Turtle] -> [Track]
tracks(cmds) = reversed(final.#saveTrace)
  where
...
example1 = (stars.#tracks.#polylines & anchors.#polyline).#drawingOf
  where
  ...
  anchors = concatenation(tracks(foreach([0,4..120], noStar)))

-- Examples from the Logo 15-word challenge
-- http://www.mathcats.com/gallery/15wordcontest.html
 
example2 = (slides.$make ++ [bullring.#tracks.#dottylines]).#slideshow
  where
  make(i,l,s) = blank
